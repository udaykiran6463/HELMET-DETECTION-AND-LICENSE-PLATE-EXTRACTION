...
def getNumberPlate(xyxy,im0,li,na):
    x1,y1,x2,y2=xyxy
    ...
    print(plate)
    path='./number_plates/'
    path1='./number_plates_text/'
    timestamp = time.strftime('%Y%m%d-%H%M%S')
    filename = path+ f'image_{timestamp}.jpg'
    filename1 = path1+ f'image_{timestamp}.txt'
...
def detect(opt):
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        if names[c]=='No Helmet'and conf>0.5:
                            flag=1
                        if names[c]=='Helmet' and conf<0.5:
                            flag=1 
                        if c==0 and conf >0.75:
                            flag=0
                        if names[c]=='LP' and flag==1:
                            st=getNumberPlate(xyxy,im0,li,na)
                            label = 'LP: '+st+" "+f'{conf:.2f}'
                            flag=0
    ...
    df['Timestamp']=na
    df['License plate']=li
    df.to_csv('License plate.csv', mode='a', header=False, index=False)
